...
Тем не менее, выполнение оператора switch по строкам может оказаться более затратным, чем по целым числам. Таким образом, лучше всего применять 
...
Чтобы позволить двум и более переменным управлять циклом for, в части инициализация и итерация цикла for разрешено 
...
он обеспечивает возможность ветвления произвольным и неструктурированным образом, что обычно затрудняет понимание и сопровождение кода, опирающегося на переходы в стиле "goto': Кроме того, "goto" препятствует некоторым оптимизациям со стороны компилятора. Тем не менее, есть несколько 
...
Имейте в виду, что использовать оператор break с меткой, которая определена не для охватывающего блока, не разрешено. Например, следующая 
...
раньше, до достижения конца тела. То есть выполнение цикла должно продолжаться, но без обработки остатка кода в его теле для конкретной итерации. По сути, это переход в конец цикла. Такое действие реализует оператор 
